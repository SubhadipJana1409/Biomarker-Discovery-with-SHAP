...
def plot_roc_confusion(
    fpr, tpr, test_auc: float, cm: np.ndarray, out_dir: Path
) -> None:
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Purples", cbar=False,
                xticklabels=["Healthy", "IBD"], yticklabels=["Healthy", "IBD"],
...
def plot_biomarker_bar(
    top_df: pd.DataFrame, out_dir: Path, n: int = 20
) -> None:
    ...
    if not has_pub:
        axes = [axes]
    fig.suptitle("Top Microbial Biomarkers — SHAP Feature Importance",
                 fontsize=14, fontweight="bold")

    colors = [RED if d == "IBD↑" else GRN for d in top["SHAP_Direction"]]
    axes[0].barh(top["Taxon"][::-1], top["Mean_SHAP"][::-1],
                 color=colors[::-1], edgecolor="#333355", linewidth=0.5, height=0.7)
    ...
    if has_pub:
        ...
        axes[1].barh(top["Taxon"][::-1], top["Published_Effect"].fillna(0)[::-1],
                     color=pub_colors[::-1], edgecolor="#333355", linewidth=0.5, height=0.7)
    ...
    legend_handles = [mpatches.Patch(color=RED, label="Enriched in IBD ↑"),
...
def plot_shap_beeswarm(
    shap_ibd: np.ndarray, X_test: np.ndarray, feature_names: list[str],
    out_dir: Path, max_display: int = 20
) -> None:
    """Fig 3 — SHAP beeswarm / summary plot."""
    fig, _ = plt.subplots(figsize=(10, 8))
    fig.patch.set_facecolor(BG)
    shap.summary_plot(shap_ibd, X_test, feature_names=feature_names,
                      max_display=max_display, show=False, plot_size=None)
    plt.title("SHAP Beeswarm — Microbial Biomarkers for IBD",
...
def plot_waterfall(
    shap_exp: shap.Explanation, sample_idx: int, out_dir: Path,
    max_display: int = 15
) -> None:
...
def plot_dependency(
    shap_ibd: np.ndarray, X_test: np.ndarray, feature_names: list[str],
    top_taxa: list[str], out_dir: Path
) -> None:
    """Fig 5 — SHAP dependency plots for top-4 taxa."""
    n = min(4, len(top_taxa))
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.patch.set_facecolor(BG)
    fig.suptitle("SHAP Dependency Plots — Top Biomarkers",
                 fontsize=14, fontweight="bold")
    ...
    for ax, taxon in zip(axes.flat, top_taxa[:n]):
        ...
        x_    = X_test[:, idx]
        s_    = shap_ibd[:, idx]
        sc    = ax.scatter(x_, s_, c=x_, cmap="RdYlGn_r",
...
def plot_shap_heatmap(
    shap_ibd: np.ndarray, y_test: np.ndarray, feature_names: list[str],
    top_taxa: list[str], out_dir: Path
) -> None:
    ...
    fig.patch.set_facecolor(BG)
    sns.heatmap(shap_mat[sort_order].T, cmap="RdBu_r", center=0,
                xticklabels=False, yticklabels=short_names, ax=ax,
                cbar_kws={"label": "SHAP Value (IBD)"}, linewidths=0)
    for i, idx in enumerate(sort_order):
        ax.add_patch(plt.Rectangle((i, -1.2), 1, 0.8,
                                    color=col_colors[idx], clip_on=False))
    ax.set_title("SHAP Heatmap — Top 15 Biomarkers × All Test Samples",
                 fontsize=13, fontweight="bold", pad=20)
    ax.tick_params(axis="y", labelsize=9)
    ax.legend(handles=[mpatches.Patch(color=RED, label="IBD"),
                        mpatches.Patch(color=GRN, label="Healthy")],
              loc="upper right", bbox_to_anchor=(1.14, 1.0))
    plt.tight_layout()
    _save(fig, out_dir / "fig6_shap_heatmap.png")


def plot_shap_vs_published(
    biomarker_df: pd.DataFrame, out_dir: Path
) -> None:
    ...
    fig.patch.set_facecolor(BG)
    colors = [RED if (pd.notna(v) and v > 0) else GRN
              for v in biomarker_df["Published_Effect"]]
    ax.scatter(biomarker_df["Published_Effect"], biomarker_df["Mean_SHAP"],
               c=colors, s=60, alpha=0.8, edgecolors="#555577", linewidth=0.5)
    for _, row in biomarker_df.head(10).iterrows():
        ax.annotate(row["Taxon"].replace("_", " "),
                    (row["Published_Effect"], row["Mean_SHAP"]),
                    fontsize=7, xytext=(5, 5), textcoords="offset points",
                    color=TEXT, alpha=0.9)
    ...
    ax.set_title("SHAP Discovery vs Published Meta-Analysis",
                 fontsize=13, fontweight="bold")
    ax.grid(True, alpha=0.3)
    ax.legend(handles=[mpatches.Patch(color=RED, label="Enriched in IBD"),
                        mpatches.Patch(color=GRN, label="Depleted in IBD")])
    plt.tight_layout()
